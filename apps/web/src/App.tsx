...
function RiskControls({ state }: { state: RewardsAppState }) {
  ...
  const rows = [
    ...
    ['Max queue share', `${Math.round(config.maxQueueShare * 100)}%`, 'neutral'],
    ['Min bid depth x', config.minSideDepthMultiplier.toFixed(1), 'neutral'],
    ['Min ask depth x', config.minAskDepthMultiplier.toFixed(1), 'neutral'],
    ['Inventory exit', config.inventoryExitEnabled ? 'enabled' : 'disabled', config.inventoryExitEnabled ? 'warn' : 'neutral'],
    ['Exit close window', `${Math.round(config.inventoryExitSecondsToClose / 60)}m`, 'neutral'],
    ['Inventory stop loss', config.maxInventoryLossPerShare.toFixed(3), 'neutral'],
    ['Extreme stop loss', config.maxExtremeInventoryLossPerShare.toFixed(3), 'warn'],
